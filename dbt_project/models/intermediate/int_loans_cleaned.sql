...
-- Second transformation layer. Takes stg_loans and:
-- 1. Fills nulls in history fields with 999
--    (999 = "no history" sentinel — standard credit scoring practice)
-- 2. Converts emp_length string to numeric years
-- 3. Averages fico_range_low/high to single fico_score
-- 4. Log-transforms annual_inc to reduce right skew
--    (income is heavily skewed — log makes it approximately normal)
-- 5. Converts earliest_cr_line to credit age in years

WITH stg AS (
    -- The ref macro below tells dbt this depends on stg_loans.
